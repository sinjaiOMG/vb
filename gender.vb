...
Public Class gender
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        connect()
        Try
            sql = "update pettype set name = N'" & TextBox3.Text & "' WHERE GID = N'" & TextBox2.Text & "'"
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Else
            Try
                sql = "insert into pettype values(N'" & TextBox2.Text & "',N'" & TextBox3.Text & "')"
                cmd = New SqlCommand(sql, cn)
                cmd.ExecuteNonQuery()
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        With YESorNO
            .Label2.Text = "ທ່ານຕ້ອງການລົບຂໍ້ມູນນີ້ບໍ່"
            .ShowDialog()
            If .Label4.Text = "ຢືນຢັນ" Then
                ' Me.Close()
                Try
                    sql = "Delete from pettype where GID = '" & (TextBox2.Text) & "'"
                    cmd = New SqlCommand(sql, cn)
                    cmd.ExecuteNonQuery()
                Catch ex As Exception
                    'messageboxss.Show("ເກີດຄວາມບໍ່ຖືກຕ້ອງລະຫວ່າງດຳເນີນການ")
    ...
    Private Sub AutoID()
        sql = "Select max(GID) from pettype"
        cmd = New SqlCommand(sql, cn)
        Try
    ...
    Private Sub ShowServiceData()
        connect()
        sql = "select GID,name from pettype"
        da = New SqlClient.SqlDataAdapter(sql, cn)
        ds = New DataSet
        da.Fill(ds, "pettype")
        DataGridView1.DataSource = ds
        DataGridView1.DataMember = "pettype"
        FormatDGVStyleStock()
    End Sub
    ...
    Public Sub FilterData(valueToSearch As String)
        Dim searchQuery As String = "SELECT * FROM pettype WHERE CONCAT(GID,name) like N'%" & valueToSearch & "%' "
