...
Public Class mCategoty
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        connect()
        Try
            sql = "update MD set name = N'" & TextBox3.Text & "' WHERE MDID = N'" & TextBox2.Text & "'"
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        With YESorNO
            ...
            If .Label4.Text = "ຢືນຢັນ" Then
                Try
                    sql = "Delete from MD where MDID = '" & (TextBox2.Text) & "'"
                    cmd = New SqlCommand(sql, cn)
                    cmd.ExecuteNonQuery()
                Catch ex As Exception
                    '  messageboxss.Show("ເກີດຄວາມບໍ່ຖືກຕ້ອງລະຫວ່າງດຳເນີນການ")
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Else
            Try
                sql = "insert into MD values(N'" & TextBox2.Text & "',N'" & TextBox3.Text & "')"
                cmd = New SqlCommand(sql, cn)
                cmd.ExecuteNonQuery()
    ...
    Sub connect()
        ...
        End Try
    End Sub
    Private Sub mCategoty_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub ShowServiceData()
        connect()
        sql = "select MDID,name from MD"
        da = New SqlClient.SqlDataAdapter(sql, cn)
        ds = New DataSet
        da.Fill(ds, "MD")
        DataGridView1.DataSource = ds
        DataGridView1.DataMember = "MD"
        FormatDGVStyleStock()
    End Sub
    ...
    Public Sub FilterData(valueToSearch As String)
        Dim searchQuery As String = "SELECT * FROM MD WHERE CONCAT(MDID,name) like N'%" & valueToSearch & "%' "
